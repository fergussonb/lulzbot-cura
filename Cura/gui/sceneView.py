...
from Cura.util.energy import energy
...
class SceneView(openglGui.glGuiPanel):
	...
	"""modified to add energy estimates to the gcode"""
	def _saveGCode(self, targetFilename, ejectDrive = False):
		...
		try:
			...
			with open(targetFilename, 'wb') as fdst:
				eString = ";Estimated Energy to run job: %d Wh" % self._energyEstimate
				fdst.write(eString)
				fdst.write("\r\n")
				while 1:
	...
	def _updateEngineProgress(self, progressValue):
		...
		if finished:
			...
			for e in xrange(0, int(profile.getMachineSetting('extruder_amount'))):
				...
				if cost is not None:
					text += '\n%s' % (cost)
				"""this is where the energy estimate was added"""
				material = profile.getProfileSetting('simpleModeMaterial')
				energyCost = energy(timeToInt(result.getPrintTime()))
				self._energyEstimate = energyCost.getRunEnergy()
				text += "\n%d Wh" % energyCost.getRunEnergy()
				if 'ABS' not in material and 'PLA' not in material and 't-glase' not in material and 'HIPS' not in material and 'Nylon' not in material:
					text += " (Custom)"
